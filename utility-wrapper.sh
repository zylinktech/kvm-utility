...
available_pools=$(virsh pool-list --all | awk 'NR>2 {print $1}')
...
vm_directory=$(virsh pool-dumpxml "$selected_pool" | grep -oP '(?<=<path>).*?(?=</path>)')
...
if [ $? -eq 0 ]; then
  ...
  whiptail --title "Error" --msgbox "Failed to create VM $VM_NAME. Check the log file at $LOG_FILE for details." Here’s the updated script with storage pool selection integrated:
